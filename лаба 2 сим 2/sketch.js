...
const bgMusic = new Audio('./jump-the-groove-20240529-014751.mp3');
...
function setup() {
    ...
    // Настройка регуляторов громкости
    musicSlider = createSlider(0, 100, 50);
    musicSlider.position(width - 150, 20);
    musicSlider.style('width', '100px');
    musicSlider.style('z-index', '9999');
    
    soundSlider = createSlider(0, 100, 50);
    soundSlider.position(width - 150, 60);
    soundSlider.style('width', '100px');
    soundSlider.style('z-index', '9999');
    
    // Подписи слайдеров
    let musicLabel = createDiv('Music');
    musicLabel.position(width - 250, 20);
    musicLabel.style('color', 'white');
    
    let soundLabel = createDiv('Sounds');
    soundLabel.position(width - 250, 60);
...
function draw() {
    ...
    if (audioEnabled) {
        ...
        deathSound.volume = soundSlider.value() / 100;
        jumpSound.volume = soundSlider.value() / 100;
        killSound.volume = soundSlider.value() / 100;
    }
